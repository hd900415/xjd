...
class RepayAction extends CommonAction
{
    ...
    public function loans()
    {
        ...
        $userModel = D("User");
        if(!empty($_GET['uidddd'])){
             $userInfo["id"] = $_GET['uidddd'];
             $userInfo["confirm"] = "0"; 
        }else{
...
//            $doquota = $userModel->getDoquota($userInfo["id"]);
            ...
            $bill = $loanbillModel->field(['status','id', 'toid', 'oid', 'delay', 'delay_day', 'repay_interest', 'repayment_time', 'add_time', 'money'])
                ->where(array("uid" => $userInfo["id"], "status" => array("IN", "0,1,4,5,6,-4")))->order("id DESC")->order("repayment_time DESC")->find();
            ...
            if ($bill) {
                ...
                $loanOrder = $loanorderModel->where(array("uid" => $userInfo['id'], "pending" => 0, "status" => 0))->order('id DESC')->order('add_time DESC')->find();
            ...
            if (!$loanOrder) {
                ...
                //应还金额=借款金额+Interest利息+手续费Commission+服务费serviceFee
           ...
           // $bill['repay_interest'] = $repayFee;
            $bill['interest'] = $bill['repay_interest'];
          
            $bill['serviceFee'] = $repayFee;
//            print_r([$money , $loanTime , $serviceRate]);
            ...
            $returnData = array(
               ...
               // "serviceFee" => $serviceFee,
                "repay_interest" => $serviceFee,
                "bill" => $bill,
                "hasOverdue" => $repaymentTime < time(),
                'doquota' => $user['quota'],
                'commission' =>  C('Commissions') ? C('Commissions') : 0,
            ...
            //$loanOrder  0待还款，1已逾期，2已还清，4 已延期，5,部分还款
         
            if($loanOrder['pending']=="2" && $bill['status']=="-4"){
                $returnData['status']=6;
            ...
            if($loanOrder['pending']=="0" && $loanOrder['status']=="0"){
                $returnData['status']=5;
                 $this->ajaxReturn(array("status" => 200,
                    "data" => $returnData,
                    "message" => "Successfully")
...
//        $this->assign("bill", $bill);
    ...
    private function renderTemplate($delay)
    {
        ...
        }else if (in_array(strtolower($_SERVER['HTTP_HOST']), ['india-moneyview.agsog.top','m.vderg.top','m32.ydtest.xyz'])) {
            //粉色 moneyview
        ...
        } else if (in_array(strtolower($_SERVER['HTTP_HOST']), ['m2.ydtest.xyz', 'm.glbierb9.top', 'mexico-samllcapital.agsog.top','m.baubap.top','m.prestamorapidoen.top','m.weeuw.top','m.gldsa.top','m.sgbkss.xyz'])) {
    ...
    public function payData()
    {
        ...
        if ($this->isPost()) {
            ...
            if (intval($daishouSwitch) == 1) { //提交到代付
                ...
                $response = $paymentModel->repayment($loanOrder, $bill, $paytype);
                if ($response['code'] == 200) {
                    $this->setAjaxResponse(200, $response['pay_url'].'&next=pay&testpay=1', 'success');
                } else {
                    $this->setAjaxResponse(500, "Payment Order Submit Failed:" . $response['msg']);
                }
            } else {
    ...
    public function payTransf()
    {
        if ($this->isPost()) {
            $url=I('url');
            $patype=I('paytype');
            if (!$url) {
                $this->setAjaxResponse(300, '', "param url id is required");
            }
            if (!$patype) {
                $this->setAjaxResponse(300, '', "param paytype id is required");
            }
            $resp=doCurl($url,['paytype'=>$patype] , 1);
            if($resp){
                $data=json_decode($resp,true);
                if($data['code']==200){
                    $this->setAjaxResponse(200, $data['data'], "successful");
                }else{
                    $this->setAjaxResponse(501, '', "server error");
                }
            }else{
                $this->setAjaxResponse(500, '', "server error");
            }
        }
    }
    /**
    ...
    public function delayTransf()
    {
        ...
        if ($this->isPost()) {
            ...
            $bill = $loanbillModel->where(array("oid" => $orderId))->find();
            if (!$loanOrder || !$bill) {
                $this->setAjaxResponse(404,'',"order or bill does not exist");
            }
            if ($loanOrder['status'] == 2 || $loanOrder['pending'] != 1) {
                $this->setAjaxResponse(403,'',"the order status does not allow operations");
            }
            $delayRate = C('PostPone_RATE');
            $processDelayData = $this->processDelay($bill, $delayRate, $loanOrder, $paytype);
            $response = $processDelayData["response"];
            if ($response) {
                if ($response['code'] == 200) {
                    ...
                    $this->setAjaxResponse(200,$response['pay_url'],"ok");
                } else {
                    $this->setAjaxResponse(500,'',"Payment Order Submit Failed:" . $response['msg']);
                }
            } else {
...
//        $list = $loanbillModel->where(['uid' => $userInfo["id"]])->select();
        ...
        foreach ($list as $key => $value) {
            ...
            /*2024-9-6修改：获取需要支付的金额*/
            //$orders[$key]['repay_money'] = floor($list[$key]["money"] + $orders[$key]['repay_interest']);
           $orders[$key]['repay_money'] = $loanorderModel->calcRepayMoney($value);
